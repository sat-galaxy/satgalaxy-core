...
extern "C"
{
    ...
    typedef void *(*satgalaxy_picosat_malloc)(void *, size_t);
    typedef void *(*satgalaxy_picosat_realloc)(void *, void *, size_t, size_t);
    ...
    /// @note Only meaningful if satgalaxy_picosat_sat returned PICOSAT_UNSATISFIABLE.
    /// @note Valid until the next satgalaxy_picosat_sat, satgalaxy_picosat_mus_assumptions, or this function call.
    /// @return Pointer to a zero-terminated minimized array of failed assumptions.
    const int *satgalaxy_picosat_mus_assumptions(PicoSATSolver *solver, void *state, void (*callback)(void *, const int *), int fix);

    /// @brief Computes a maximal satisfiable subset of assumptions.
    /// @param solver Pointer to the PicoSATSolver.
    /// @note Requires assumptions set and satgalaxy_picosat_sat called with satgalaxy_picosat_inconsistent checked.
