...
int cadical_error (CaDiCaLSolver *solver) { return solver->last_error; }
...
const char *cadical_error_message (int code) {
  ...
  case 101:
    return "external solver not initialized";
  case 102:
    return "internal solver not initialized";
  case 103:
    return "solver in invalid state";
  case 104:
  ...
  case 107:
    return "solver is not in steady state";
  case 108:
    return "invalid zero file argument";
  case 109:
    return "already tracing API calls using environment variable "
           "'CADICAL_API_TRACE'";

  case 110:
    return "called trace_api_calls twice";
  case 111:
    return "can only set option 'set' right after initialization";
  case 112:
    return "can only set configuration right after initialization";
  case 113:
    return "first argument 'lits' zero while second argument 'size' not";
  case 114:
    return "negative number of simplification rounds";
  case 115:
    return "can only get value in satisfied state";
  case 116:
    return "can only flip value in satisfied state";
  case 117:
    return "can only flip when no external propagator is present";
  case 118:
    return "can only get failed assumptions in unsatisfied state";
  case 119:
    return "can only determine if constraint failed in unsatisfied state";
  case 120:
    return "can not connect zero terminator";
  case 121:
    return "can not connect zero learner";
  case 122:
    return "can not connect zero fixed listener";
  case 123:
    return "can not connect zero propagator";
  case 124:
    return "can not melt completely melted literal";
  case 125:
    return "can only start proof tracing right after initialization";
  case 126:
    return "proof is not traced";
  case 127:
    return "proof trace already closed";
  case 128:
    return "can only start proof tracing right after initialization";
  case 129:
    return "proof is not traced";
  case 130:
    return "can not disconnect zero tracer";
  case 131:
    return "can only conclude in satisfied, unsatisfied or STEADY state";
  case 132:
    return "can only read DIMACS file right after initialization";
  case 133:
    return "target solver already modified";
  case 202:
  ...
  case 400:
    return "can not reuse molten literal";
  case 401:
    return "unassigned variable";
  case 402:
    return "inconsistently assigned literals";
  case 403:
    return "assumption falsified";
  case 404:
    return "assumption unassigned";
  case 405:
    return "constraint not satisfied";
  case 406:
    return "failed assumptions do not form a core";
  case 407:
    return "default value lower than minimum in 'options.hpp'";
  case 408:
    return "default value larger than maximum in 'options.hpp'";
  case 409:
    return "invalid option order in 'options.hpp'";
  case 410:
    return "learned empty clause but got solution";
  case 411:
    return "learned unit contradicts solution";
  case 412:
    return "can not trace API calls of two solver instances using "
           "environment variable 'CADICAL_API_TRACE'";
  case 413:
    return "failed to open file to trace API calls using environment "
           "variable 'CADICAL_API_TRACE'";
  default:
...
int cadical_vars (CaDiCaLSolver *s) { CALL_FN (0, s->solver->vars, ); }

void cadical_add_clause (CaDiCaLSolver *s, const int ps[], size_t length) {
  CALL_FN (, s->solver->clause, ps, length);
}
void cadical_add_empty_clause (CaDiCaLSolver *s) {
  CALL_FN (, s->solver->add, 0);
}

void cadical_add (CaDiCaLSolver *s, int lit) {
  CALL_FN (, s->solver->add, lit);
}
...
int cadical_set_opt_bumpreasondepth (CaDiCaLSolver *s,
                                     int bumpreasondepth) {
...
int cadical_set_opt_checkassumptions (CaDiCaLSolver *s,
                                      int checkassumptions) {
  s->last_error = 0;
  return cadical_set_option (s, "checkassumptions", checkassumptions);
}

int cadical_set_opt_checkconstraint (CaDiCaLSolver *s,
                                     int checkconstraint) {
  s->last_error = 0;
...
int cadical_set_opt_chronoreusetrail (CaDiCaLSolver *s,
                                      int chronoreusetrail) {
  s->last_error = 0;
...
int cadical_set_opt_conditionmaxeff (CaDiCaLSolver *s,
                                     int conditionmaxeff) {
...
int cadical_set_opt_conditionmaxrat (CaDiCaLSolver *s,
                                     int conditionmaxrat) {
...
int cadical_set_opt_conditionmineff (CaDiCaLSolver *s,
                                     int conditionmineff) {
...
int cadical_set_opt_conditionreleff (CaDiCaLSolver *s,
                                     int conditionreleff) {
...
int cadical_set_opt_decomposerounds (CaDiCaLSolver *s,
                                     int decomposerounds) {
...
int cadical_set_opt_eagersubsumelim (CaDiCaLSolver *s,
                                     int eagersubsumelim) {
...
int cadical_set_opt_exteagerreasons (CaDiCaLSolver *s,
                                     int exteagerreasons) {
  s->last_error = 0;
...
int cadical_set_opt_instantiateclslim (CaDiCaLSolver *s,
                                       int instantiateclslim) {
...
int cadical_set_opt_instantiateocclim (CaDiCaLSolver *s,
                                       int instantiateocclim) {
...
int cadical_set_opt_instantiateonce (CaDiCaLSolver *s,
                                     int instantiateonce) {
  s->last_error = 0;
...
int cadical_set_opt_reducetier1glue (CaDiCaLSolver *s,
                                     int reducetier1glue) {
...
int cadical_set_opt_reducetier2glue (CaDiCaLSolver *s,
                                     int reducetier2glue) {
...
int cadical_set_opt_restartreusetrail (CaDiCaLSolver *s,
                                       int restartreusetrail) {
  s->last_error = 0;
...
int cadical_set_opt_stabilizefactor (CaDiCaLSolver *s,
                                     int stabilizefactor) {
...
int cadical_set_opt_stabilizemaxint (CaDiCaLSolver *s,
                                     int stabilizemaxint) {
